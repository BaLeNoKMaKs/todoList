...
export default function Tasks({
  ...
  const onEditT = () => {
    ...
    if (newName) {
      Axios.patch("http://localhost:3000/lists/" + item.id, {
        text: newName,
      })
        .then(() => {
          onEditTitle(item.id, newName);
        })
  ...
  return (
    <div className="task-block">
      <div className="task-block__info">
        <p className="task-block__title" style={{ color: item.color.hex }}>
          {item.text}
        </p>
        <div className="task-block__icon" onClick={onEditT}>
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
      ...
      <div className="task-block__tasks">
        {item.tasks && item.tasks.length > 0
          ? item.tasks.map((element) => {
              return (
                <div className="task-wrapper">
                  <label key={element.id} className="task-block__task">
                    <input
                      onChange={(e) =>
                        onCompleteTask(item.id, element.id, e.target.checked)
                      }
                      checked={element.completed}
                      type="checkbox"
                      className="task-block__checkbox"
                    />
                    <span className="task-block__fake-checkbox">
                      <svg
                        width="11"
                        height="8"
                        viewBox="0 0 11 8"
                    ...
                    <span className="task-block__text">{element.text}</span>
                  </label>
                  <div className="task-block__actions">
                    <div
                      className="task-block__action"
                      onClick={() =>
                        onEditTask(item.id, {
                          id: element.id,
                          text: element.text,
                    ...
                    <div
                      className="task-block__action"
                      onClick={() => onRemoveTask(item.id, element.id)}
                    >
                      <svg
                        width="10"
                        height="10"
          ...
          : "Задачи отсутствуют"}
        <AddTask key={item.id} list={item} onAddTask={onAddTask} />
      </div>
    </div>
  );
}
